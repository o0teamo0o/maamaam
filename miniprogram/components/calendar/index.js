...
import mergeOptionsToData from './mergeOptionsToData'
...
Component({
  ...
  methods: {
    ...
    open(opts = {}) {
      ...
      this.setSelectValue(options.selectValue)
      this.setDayCycle(options.dayCycle)
    ...
    monthHTML(date, offset) {
      ...
      if (this.data.minDate) {
        tempMinDate = new Date(this.data.minDate).setHours(0, 0, 0, 0);
        console.log("tempMinDate:", tempMinDate)
      }
      var tempMaxDate
      if (this.data.maxDate) {
        tempMaxDate = new Date(this.data.maxDate).setHours(23, 59, 59, 999);
      }
      ...
      let dayDate, currentValues = [],
        ...
        minDate = this.data.minDate ? new Date(tempMinDate).getTime() : null,
        maxDate = this.data.maxDate ? new Date(tempMaxDate).getTime() : null
      ...
      for (let i = 1; i <= rows; i++) {
        ...
        for (let j = 1; j <= cols; j++) {
          ...
          // Disabled
          if ((minDate && dayDate < minDate - 1) || (maxDate && dayDate > maxDate)) {
            type.disabled = true
          }
    ...
    setMonthsHTML() {
      ...
      if (this.data.selectValue) {
        layoutDate = this.data.selectValue && this.data.selectValue.length ? this.data.selectValue[0] : new Date().setHours(0, 0, 0, 0)
      } else {
        layoutDate = this.data.value && this.data.value.length ? this.data.value[0] : new Date().setHours(0, 0, 0, 0)
      }
    ...
    updateValue() {
      ...
      if (this.data.value && this.data.value.length) {
        ...
        if (this.data.dayCycle > 1) {
          var startDate = DataUtl.fromCustomDate(this.data.value[0], 0, "-");
          var endDate = DataUtl.fromCustomDate(this.data.value[0], parseInt(this.data.dayCycle) - 1, "-");
          var selectedDateArr = DataUtl.getDateBetween(startDate, endDate);

          for (let ii = 0; ii < selectedDateArr.length; ii++) {
            const valueDate = new Date(selectedDateArr[ii])
          ...
          const valueDate = new Date(this.data.value[0])
          const valueYear = valueDate.getFullYear()
          const valueMonth = valueDate.getMonth()
          const valueDay = valueDate.getDate()
